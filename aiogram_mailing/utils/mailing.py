...
class MailingFunctions:
    ...
    def __init__(
            ...
            mailing_database: Database
    ) -> None:
        self._bot = bot
        self._user_db = data_source
        self._db = mailing_database

    async def get_menu_message(self) -> list[str, InlineKeyboardMarkup]:
        """Get mailing menu message by mailing info."""
        mailing_info = await self._db.get_mailing()
        ...
        if mailing_info.content_type == CT.TEXT:
            ...
            vars_ = [users_count, mailing_info.alive_users, mailing_info.died_users,
                     ...
                     mailing_info.button_text, mailing_info.button_link]
            return MT.mailing_menu_types["video_note"].format(*vars_), await MK.mailing_menu_keyboard(
                is_video_note=True
            )
    ...
    async def send_mailing_message(
        ...
        if content_type not in [CT.TEXT, CT.VIDEO_NOTE]:
            if mailing_info.text != "Не установлен":
                await content_types[content_type](
                    user_id, mailing_info.media, caption=mailing_info.text, reply_markup=k.as_markup()
                )
            else:
                await content_types[content_type](user_id, mailing_info.media, reply_markup=k.as_markup())
        elif content_type == CT.TEXT:
            await content_types[content_type](
                user_id, mailing_info.text, reply_markup=k.as_markup(),
                disable_web_page_preview=mailing_info.link_preview
            )
