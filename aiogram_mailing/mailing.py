...
async def register_handlers(
    ...
    mailing_router.message.filter(admin_filter)
    mailing_router.callback_query.filter(admin_filter)
    menu.dispatcher.include_router(mailing_router)
    ...
    async def mailing_menu(message: types.Message, state: FSMContext):
        ...
        await message.answer(text=mailing_info[0], reply_markup=mailing_info[1], disable_web_page_preview=True)

    @mailing_router.callback_query(F.data == MD.Back.back_to_mailing)
    ...
    @mailing_router.callback_query(F.data == MD.Menu.disable_webpage_preview)
    async def disable_webpage_preview(call: types.CallbackQuery):
        await db.update_link_preview()
        await call.answer(MT.done, show_alert=True)
        mailing_info = await sender.get_menu_message()
        await call.message.edit_text(text=mailing_info[0], reply_markup=mailing_info[1], disable_web_page_preview=True)

    @mailing_router.callback_query(F.data == MD.Menu.reset)
    ...
    @mailing_router.callback_query(F.data == MD.Menu.media)
    ...
    @mailing_router.message(
        MailingState.media, F.content_type.in_(
            [CT.AUDIO, CT.VIDEO, CT.VOICE,
    ...
    async def update_media(message: types.Message, state: FSMContext):
        ...
        await message.bot.edit_message_text(
            chat_id=message.chat.id, message_id=message_id, text=mailing_info[0], reply_markup=mailing_info[1],
            disable_web_page_preview=True
        )
        await state.clear()

    @mailing_router.callback_query(F.data == MD.Menu.text)
    async def await_update_text(call: types.CallbackQuery, state: FSMContext):
        message = await call.message.edit_text(
            text=MT.mailing_text, reply_markup=await MK.back_to_mailing_keyboard(), disable_web_page_preview=True
        )
        await state.update_data(message_id=message.message_id)
        await state.set_state(MailingState.text)

    @mailing_router.message(MailingState.text)
    ...
    @mailing_router.callback_query(F.data == MD.Menu.button)
    async def edit_button_menu(call: types.CallbackQuery, state: FSMContext):
        await call.message.edit_text(
            text=MT.mailing_button_menu, reply_markup=await MK.mailing_button_edit_keyboard(),
            disable_web_page_preview=True
        )
        await state.update_data(message_id=call.message.message_id)

    @mailing_router.callback_query(F.data.in_([MD.EditButton.button_link, MD.EditButton.button_text]))
    async def await_update_button(call: types.CallbackQuery, state: FSMContext):
        ...
        await call.message.edit_text(
            text=types[call.data][1], reply_markup=await MK.back_to_mailing_keyboard(), disable_web_page_preview=True
        )
        await state.update_data(button=call.data)
        await state.set_state(types[call.data][0])

    @mailing_router.message(
        MailingState.button_link, F.content_type.in_(CT.TEXT),
        F.text.regexp(r'https?://([a-zA-Z0-9.-]+)\.([a-zA-Z]{2,})\S*')
    )
    ...
    @mailing_router.message(MailingState.button_text, F.content_type.in_(CT.TEXT))
    ...
    @mailing_router.callback_query(F.data == MD.Menu.view_message)
    async def preview_mailing(call: types.CallbackQuery):
    ...
    @mailing_router.callback_query(F.data == MD.Menu.start)
    async def get_ready_to_start(call: types.CallbackQuery):
        users_count = await menu.data_source.get_users_count()
        await call.message.edit_text(
            MT.mailing_sure.format(users_count), reply_markup=await MK.mailing_sure_keyboard(),
            disable_web_page_preview=True
        )

    @mailing_router.callback_query(F.data == MD.StartMailing.sure)
    async def mailing(call: types.CallbackQuery):
        users = await menu.data_source.get_user_ids()
        mailing_info = await db.get_mailing()
        content_type = await db.get_content_type()
        if content_type == 'text' and mailing_info.text == 'Не установлен':
            await call.answer(MT.mailing_not_set, show_alert=True)
        ...
        await call.message.edit_text(
            MT.mailing_end.format(result[0], result[1]), reply_markup=await MK.back_to_mailing_keyboard()
        )
